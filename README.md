...
In VS Code, the `PackY: Clean` task can also be used to run these commands.

Then, to **build PackY**, run the following commands:
...
- To **build** the project (automatically updates the PyProject file and the translation files, then compiles resources (QRC files), UI files, and QML files into Python files, and TS files into a binary form (QM files)):
  ...
  - VS Code task: `PackY: Build` (also calls tasks `PackY: Update PyProject file` and `PackY: Update translation files` beforehand).
  - **Note:** the `pyside6-project build` Qt command implicitly calls the `pyside6-lrelease`, `pyside6-uic` and `pyside6-rcc` commands.
  - [pyside6-project command documentation](https://doc.qt.io/qtforpython-6/tools/pyside-project.html).
  - [pyside6-lrelease command documentation](https://doc.qt.io/qtforpython-6/tools/pyside-lrelease.html) and [its options](https://doc.qt.io/qt-6/linguist-lrelease.html)
...
- To **update PyProject file**, and more specifically the 'tool.pyside6-project.files' section in the `pyproject.toml` file (automatically called by the build command):
...
- To **update translation files** (scan the source code to extract strings and populate the translation (.ts) files in the `i18n/` directory; automatically called by the build command):
  ...
  - **Note:** this command does not rely on the `pyproject.toml` file to identify source files, but instead performs a direct scan of the file system, so it is not necessary to update it beforehand.
